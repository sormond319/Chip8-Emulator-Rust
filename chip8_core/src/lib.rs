...
impl Emu {
    ...
    if pc_usize >= RAM_SIZE - 1 {
        panic!("PC out of bounds for 2‑byte fetch: {} (RAM size {})", self.pc, RAM_SIZE);
    }
    let higher_byte = self.ram[pc_usize] as u16;
    let lower_byte  = self.ram[pc_usize + 1] as u16;
